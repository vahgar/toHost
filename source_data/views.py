...
from .models import City,Area,Street,Category,SubCategory
...
def postandresp(request):
	if request.method == "POST":
		...
		if form.is_valid() and form1.is_valid():
			...
			data = business_entity.objects.filter(Q(city__name=city),Q(area__name=area),Q(locality__name=street),Q(category__name=category),Q(subcategory__name=subCategory))
			context = {'data':data}
